...
class Application
{
    ...
    private function CheckPerformTransaction()
    {
        ...
        /*if ($found && ($found->state == Transaction::STATE_CREATED || $found->state == Transaction::STATE_COMPLETED)) {
            $this->response->error(
                ...
                PaycomException::message(
                    'Для этого заказа есть другая активная / завершенная транзакция.',
                    'Для этого заказа есть другая активная / завершенная транзакция.',
                    'Для этого заказа есть другая активная / завершенная транзакция.'
                )
            );
        ...
        if ($found && ($order->state == Order::STATE_WAITING_PAY)) {
            $this->response->error(
                PaycomException::ERROR_INVALID_ACCOUNT,
                PaycomException::message(
                "Заказ уже оплачен или в ожидании платежа",
    			"Заказ уже оплачен или в ожидании платежа",
    			"Заказ уже оплачен или в ожидании платежа"
                )
            );
        ...
        if ($found && ($order->state == Order::STATE_PAY_ACCEPTED || $order->state == Order::STATE_CANCELLED)) {
            $this->response->error(
                PaycomException::ERROR_INVALID_ACCOUNT,
                PaycomException::message(
                "Заказ уже оплачен или отменен",
    			"Заказ уже оплачен или отменен",
    			"Заказ уже оплачен или отменен"
                )
            );
        ...
        $this->response->send(Array('allow' => true));
    }
...
//$this->log($found->create_time);
		//	$this->log(Format::datetime2timestamp($found->create_time));
        // todo: Prepare and send found transaction
	
        $this->response->send(Array(
            'create_time'  => strtotime($found->create_time) * 1000,
            'perform_time' => strtotime($found->perform_time) * 1000,
            'cancel_time'  => strtotime($found->cancel_time) * 1000,
    ...
    private function CreateTransaction()
    {
        ...
        $transaction = new Transaction();
        $found       = $transaction->find(Array('account' => $this->request->params['account']));
        if ($found) {
        ...
        if ($found) {
            ...
            }/*elseif ($found->paycom_transaction_id == $this->request->params['id']) { 
                //$found->cancel(Transaction::REASON_CANCELLED_BY_TIMEOUT);
                $this->response->error(
                    PaycomException::ERROR_INVALID_ACCOUNT,
                    PaycomException::message(
            ...
            $create_time                        = Format::timestamp();
              ...
              //  $this->log($found->create_time.'--1');
		//	$this->log(Format::timestamp2milliseconds($found->create_time).'--1-');
            }
            ...
            $transaction->create_time           = Format::timestamp2milliseconds($create_time);
			...
			//$this->log($create_time);
			//$this->log(Format::datetime2timestamp($create_time));
            // send response
            $this->response->send([
    ...
    private function CancelTransaction()
    {
        ...
        switch ($found->state) {
            ...
            case Transaction::STATE_CANCELLED_AFTER_COMPLETE:
                $this->response->send([
                    'transaction' => $found->id,
                    //'cancel_time' => Format::timestamp2seconds($found->cancel_time),
                    'cancel_time' => strtotime($found->cancel_time) * 1000,
                    'state'       => $found->state,
                ]);
                break;
            ...
            case Transaction::STATE_CREATED:
                ...
                $order = new Order($this->request->id);
                $this->request->params['order_id'] = $found->order_id;
