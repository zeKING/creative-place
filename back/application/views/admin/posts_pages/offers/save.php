...
<!--<div class="tabbable">
        ...
        <? $i=1; foreach ($settings['languages']->value as $key => $val): ?>
            <div class="tab-pane fade <?=($i==1) ? 'show active' : ''?>" id="tab<?= $i ?>">
                ...
                <div class="form-group">
                    ...
                    <div class="controls">
                        <input id="title_no" name="title[<?= $key ?>]" class="form-control input-xlarge focused titles_no" type="text" value="<?= set_value('title[' . $key . ']', _t(@$post->title, $key)) ?>">
                    </div>
                </div>
 ...
 <div class="form-group">
		<label class="control-label" for="focusedInput">Процент скидки</label>
		<div class="controls">
			<input id="discount" name="discount" class="span3 form-control" type="number" value="<?=set_value('discount', $post->discount)?>" required>
		</div>
	</div>
  <?php $facture = getOptionsData(array('group' => 'products', 'status' => 'active', 'limit' => 50000000));?>
    <div class="form-group">
       <label class="control-label"  for="select-state">Товары:</label>
       <div class="controls">
          <select id="select-state7" name="tags[]"  class="demo-default"  multiple=''>
             ...
             <option value="<?=$item->id?>" ><?=_t($item->title)?>  <?php if(getOffers($item->id, 'offers', 'discount')){ ?> <strong>&nbsp; (Используется) </strong> <?php }?></option>
...
$this->load->view('admin/components/status'); ?>
