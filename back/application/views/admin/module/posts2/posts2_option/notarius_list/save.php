...
<div class="tabbable">    
    ...
    <div class="tab-content">
        ...
        <div class="tab-pane  fade <?=($i==1) ? 'show active' : ''?>" id="tab<?= $i ?>">
            <div class="form-group">
                ...
                <div class="controls">
                    <input name="title[<?= $key ?>]" class="form-control input-xlarge focused titles" type="text" value="<?= set_value('title[' . $key . ']', _t(@$post->title, $key)) ?>">
                </div>
            </div>
            <div class="form-group" style="display: none">
                <label class="control-label" for="focusedInput">Категория новости</label>
                <div class="controls">
                    <input name="category_title[<?= $key ?>]" class="form-control form-control input-xlarge focused" type="text" value="<?= set_value('category_title[' . $key . ']', _t(@$post->category_title, $key)) ?>">
                </div>
            </div>
            <!--<div class="form-group">
	...
	<div class="form-group">
		...
		<div class="controls">
			<select id="category_id" name="category_id2" class="form-control input-xlarge focused">
...
/*
      <!-- <div class="form-group">
       ...
       <div class="controls">
		...
		<div class="controls">
			<select name="option" class="form-control form-control input-xlarge focused">
...
<div class="form-group">
    ...
    <div class="controls">
        <input <?php if (!$post->alias) { ?>id="alias" <?php } ?> name="alias" class=" form-control span3 validate[required,ajax[check_alias2]]" type="text" value="<?= set_value('alias', $post->id) ?>">
    </div>
</div>
<?//php $this->load->view('admin/components/meta'); validate[required,ajax[check_alias2]] ?>
<?php $this->load->view('admin/components/status'); ?>

<div class="form-group" style="display: none;">
    <div class="controls">
        <a href="#myModalPosts" role="button" class="btn btn-info" data-toggle="modal"><i class="icon-file icon-white"></i> Фото</a>
    </div>
</div>
