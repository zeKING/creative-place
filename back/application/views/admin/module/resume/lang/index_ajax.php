            ...
            <tr id="item-<?=$post->id?>">
             <td><a class="btn btn-mini move" href="#" title="Перемещать"><i class="fa fa-arrows"></i></a></td>
              ...
              <div class="btn-group sort_order_block">
                 <?=form_open('admin/resume/sort_order_posts', array('style' => 'margin-bottom: -10px;'))?>
...
<!--         </td>-->
              ...
              <?php if (isset($_GET['page'])) {
                $page = $_GET['page'];
              ?>
               <a href="<?= site_url("admin/resume/save/{$sel}/$post->id/$page") ?>" class="btn btn-small btn-info"><i class="icon-edit icon-white"></i></a>

             <?php } else { ?>
               <a href="<?= site_url("admin/resume/save/{$sel}/$post->id") ?>" class="btn btn-small btn-info"><i class="icon-edit icon-white"></i></a>
             <?php } ?>
              </div>
            </td>
             <td>
        ...
        <?php $checked = ($post->status == 'active') ? 'checked="checked"' : '';?>
            ...
            <td>
               <!-- <div class="btn-group">
                    <a href="<?=site_url('admin/resume/delete/'.$post->id)?>" class="btn btn-small delete delete-btn"><i class="icon-trash icon-white"></i></a>
                </div>-->
            </td>
        </tr>
    <? endforeach ?>
