<div id="myModalPosts" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <?=form_open_multipart('admin/posts2_media/save', array('id'=>'media_form_posts2'))?>
        ...
        <div class="progress progress-info progress-striped">
            ...
            <ul class="thumbnails" id="media_list2"> <?php if(@$media_files){?>
                    ...
                    <?php if($mf->category == 'video') {?>
                        ...
                        <div class="btn-toolbar pull-right">
                            <div class="btn-group">
                                ...
                                <a class="btn btn-mini <?=($mf->is_main) ? 'btn-info' : ''?> ajax_set_main" href="<?=base_url('admin/posts2_media/set_main/'.$mf->id)?>" title="Сделать Главным"><i class="fa fa-arrow-up"></i></a>
                                <a class="btn btn-mini btn-warning ajax_delete" href="<?=base_url('admin/posts2_media/delete/'.$mf->id)?>" title="Удалить"><i class="fa fa-trash-o"></i></a>
...
$('#file').change(function() { 
    $('#media_form_posts2').submit(); 
});

$('#media_form_posts2').ajaxForm({
    beforeSend: function() {
        status.empty();
        var percentVal = '0%';
	...
	if(group == 'music')
	{
		var temp = '<li class="thumb" data-id='+id+'><a href="" class="thumbnail tooltips" data-toggle="tooltip" data-placement="top" title="'+orig_name+'" data-original-title='+orig_name+'><i class="fa fa-music fa-6 icons-music-media"></i></a> \
			<div class="btn-toolbar pull-right"><div class="btn-group">       <a class="btn btn-mini move" href="#" title="Перемещать"><i class="fa fa-arrows"></i></a> \
					<a class="btn btn-mini ajax_set_main" href="<?=base_url("admin/posts2_media/set_main/'+id+'")?>" title="Сделать Главным"><i class="fa fa-arrow-up"></i></a> \
					<a class="btn btn-mini btn-warning ajax_delete" href="<?=base_url("admin/posts2_media/delete/'+id+'")?>" title="Удалить"><i class="fa fa-trash-o"></i></a> <div class="btn btn-mini check"><input type="checkbox"  value='+id+' id='+id+'  name="img[]"><label for='+id+'><span></span></label></div> \
			</div></li>';
	}
  <?php } else {?>
  	if(group == 'video')
...
$("#media_list2")
    .sortable({
        ...
        stop: function( event, ui ) {
            ...
            $.post(base_url + 'admin/posts2_media/sort', {media_files: JSON.stringify(thumbs)});
        }
    });
    /*.selectable({
        handle: ".thumbnail",
...
$(document).on('click', '.delete_image', function(){
		if (!confirm("Вы уверены ?")) {
	    	...
	    	$('#media_list2').html('<li  style="float: none;"><div class="loader" style="margin: 35px auto;width: 0;padding-bottom: 1px;"><div class="loader-inner ball-spin-fade-loader"><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div></div></li>');
    $.ajax({
        type: 'post',
        url: '<?=base_url()?>'+'admin/posts2/delete_image/<?=@$post->id?>',
        success: function(data){
         //$('#close').addClass('none');
           // $('.collapse').removeClass('in');
            $('#media_list2').html(data.result);
...
$(document).on('click', '.delete_image_select', function(){
  if ($('input:checkbox:checked').prop("checked")) {
    ...
    $.ajax({
        type: 'post',
        url: '<?=base_url()?>'+'admin/posts2/delete_image_select',
