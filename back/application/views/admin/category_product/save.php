			...
			<? if (isset($categories)) : ?>
				...
				<? foreach ($categories as $category) : ?>
					<? if ($category->id !== $post->id) : ?>
						<option value="<?= $category->id ?>" <? if ($category->id == $post->category_id) echo ('selected="selected"'); ?>><?= ($category->title) ? _t($category->title) : 'нет заголовка' ?></option>
					<? endif; ?>
				<? endforeach ?>
...
<div class="form-group">
	<label class="control-label" for="focusedInput">Иконка (fonts)</label>
	<div class="controls">
		<input id="icon" name="icon" class="form-control span3" type="text" value="<?= set_value('icon', $post->icon) ?>">
	</div>
</div>
