  ...
  <div class="tabbable">    
    ...
    <div class="tab-content">
        ...
        <div class="tab-pane  fade <?=($i==1) ? 'show active' : ''?>" id="tab<?= $i ?>">
            <div class="form-group">
                ...
                <div class="controls">
                    <input id="title" name="title[<?= $key ?>]" class="form-control input-xlarge focused titles" type="text" value="<?= set_value('title[' . $key . ']', _t(@$post->d_title, $key)) ?>">
                </div>
            </div>
	...
	<div class="controls">
		<select name="status" class="form-control input-xlarge focused">
			...
			<option value="inactive" <?= ($post->d_status == 'inactive') ? 'selected' : '' ?>> <?= lang('inactive') ?> </option>
		</select>
