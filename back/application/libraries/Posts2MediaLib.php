...
Class Posts2MediaLib
{
	...
	public function single_upload($folder)
	{
		...
		$config['allowed_types'] = 'gif|jpg|jpeg|png|mp3|mp4|pdf|djvu|doc|docx|xlsx|xl|word|pptx|ppt|csv|xls';
  	...
  	public function single_upload_profile($folder)
	{
		...
		$config['allowed_types'] = 'gif|jpg|jpeg|png|mp3|mp4|pdf|djvu|doc|docx|pptx|ppt|csv|xls';
	...
	{
		$media = $this->ci->db->get_where('media2', array('id'=>$id))->row();

		if ($media)
		{
			@unlink( "./uploads/{$media->category}/{$media->url}" );

			$this->ci->db->delete('media2', array('id'=>$id));
		}
	}

	public function set_main($media_id)
	{
		$media = $this->ci->db->get_where('media2', array('id'=>$media_id))->row();
