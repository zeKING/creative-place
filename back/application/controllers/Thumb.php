...
class Thumb extends CI_Controller {
	...
	public function view()
	{
		...
		$this->_file =  '.'.$params['src']; //The path to the source image
	...
	public function watermark($group, $file)
	{
		$this->load->library('MediaLib');

		echo $this->medialib->watermark($group.'/'.$file);
	}
   public function mywatermark($width = 700, $height = 400, $group, $file)
    {
        $this->load->library('MediaLib');

        echo $this->medialib->mywatermark($width, $height,$group.'/'.$file);
    }
    public function mywatermarkimg($width = 700, $height = 400, $group, $file)
    {
        $this->load->library('MediaLib');

        echo $this->medialib->mywatermarkimg($width, $height,$group.'/'.$file);
    }
	...
	protected function _create_image($newWidth, $newHeight, $crop)
	{
        ...
        if ($newWidth > $srcWidth && $newHeight > $srcHeight) 
        	$this->_error('Image width/size is less then you set');
